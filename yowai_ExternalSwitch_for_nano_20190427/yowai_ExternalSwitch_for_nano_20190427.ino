...
void reset() {
  ...
  for (int i = 0; i <= ELE_NUM; i++)digitalWrite(i, LOW);
...
byte BUTTON(byte pin_num) {
  ...
  if (!digitalRead(s_pin[pin_num]))//スイッチが押されていないとき
  {
    ...
    if (gauge[pin_num] >= PUSH_SHORT) sw_status = 255;//ゲージが閾値を超えたらSWを255にする
    gauge[pin_num] = 0;//ゲージを0にする
  }
  //return value of switch status
  if (gauge[pin_num] >= PUSH_SHORT) sw_status = 1;//ゲージが閾値を超えたらSWを1にする
...
//          break;
  ...
  // シリアル解析-------------------------------------------------------
  if ( Serial.available() > 0 ) {//シリアルモニタに文字が入力されていたら
    ...
    if ( str == "air") {//エアー
      ...
      //read_ini_flag = false;
      //release_flag = false;
    } else if ( str == "airtime" ) {//旧エアー
      pumpTest(); 
    } else if (str == "r") {//リセット
      reset();
      //exit(1);
    } else if (str == "p") { //エアーの最大値を設定 [p,設定したい値]でOK
      air_value(command[0]);
    } else if (str == "collect") {//回収用水袋のテスト
      collect_flag = true;//停止時はｒコマンド入力
      CollectWaterTest();
    } else if (str == "water") {//水のテスト
      WaterTest();
    } else if(str == "cooling_water") {//冷却水のテスト
      CoolingWaterTest();
    } else if (str == "vibration") {//振動子のテスト
      VibrationTest();
    } else if (str == "fan") {//ファンのテスト
      FanTest();
    } else if (str == "neck") {//首のテスト
      NeckTest();
    } else {
      Serial.print("> error [");
      Serial.print(str);
